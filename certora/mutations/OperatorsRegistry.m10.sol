...
contract OperatorsRegistryV1 is IOperatorsRegistryV1, Initializable, Administrable {
    ...
    /// @inheritdoc IOperatorsRegistryV1
    // @Mutation - add another index such that any valid validator can change another validator's state
    function addValidators(uint256 _index, uint32 _keyCount, bytes calldata _publicKeysAndSignatures, uint256 _index2)
        external
        onlyOperatorOrAdmin(_index2)
    {
        if (_keyCount == 0) {
    ...
    /// @inheritdoc IOperatorsRegistryV1
    // @Mutation - add another index such that any valid validator can change another validator's state
    function removeValidators(uint256 _index, uint256[] calldata _indexes, uint256 _index2) external onlyOperatorOrAdmin(_index2) {
