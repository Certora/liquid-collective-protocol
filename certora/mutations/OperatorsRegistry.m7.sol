...
contract OperatorsRegistryV1 is IOperatorsRegistryV1, Initializable, Administrable {
        ...
        returns (bytes[] memory publicKeys, bytes[] memory signatures)
    {
        ...
        for (uint256 idx = 0; idx < fundableOperatorCount; ++idx) {
            // @Mutation - use the same key several times. 
            // Probably hard to catch, but should break one of the advanced correltation properties.
            uint256 iddx = 0;
            ...
            if (operators[idx].picked > 0) {
                (bytes[] memory _publicKeys, bytes[] memory _signatures) =
                    // @Mutation - changed here. caused by using the keys of the same operator.
                    //           - A similar mutation could be done with using a hard-coded index for funded, but it's probably hard enough as it is
                    ValidatorKeys.getKeys(operators[iddx].index, operators[iddx].funded, operators[iddx].picked);
