    ...
    function _reportWithdrawToRedeemManager() internal override {
        ...
        if (underlyingAssetBalance > 0 && totalSupply > 0) {
            ...
            if (suppliedRedeemManagerDemandInEth > 0) {
                ...
                // @Mutation - use _MintRawShares instead of burn
                // break several protperties including rate is stable (for this step) and monotinicity of LsETH to ETH
                // _burnRawShares(address(redeemManager_), suppliedRedeemManagerDemand);
                _mintRawShares(address(redeemManager_), suppliedRedeemManagerDemand);
